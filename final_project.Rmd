...
```{r}
set.seed(123)
sample_indices <- sample(1:nrow(numerical), size = 1 * nrow(numerical))
sampled_numerical <- numerical[sample_indices, ]
sampled_cardio <- as.numeric(as.character(cardio$cardio[sample_indices]))
...
pairs(
  ...
  pch = 'o',
  bg = custom_colors[cardio + 1],
  col = custom_colors[cardio + 1],
  main = "\n",
  row1attop = TRUE,
  gap = 1,
  cex.labels = 1,
...
Furthermore both of these variables are *subjective*, meaning that the answer was at the respondant's discretion. This means one could straight up lie, or even, having quit recently, answer `no` even though the effects of alcohol consumption or smoking were already present. 
