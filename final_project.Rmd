...
<<<<<<< HEAD
# Use the train() function correctly
log_model_all <- train(
  cardio ~ .,                    # Formula
  data = cardio,             # Dataset
  ...
  metric = "Accuracy"            # Appropriate metric for classification
)
...
>>>>>>> 6f52747bab1f4707346e46308ab5f26dd1263b24
  geom_tile() +
...
# Get predicted probabilities
predictions <- predict(log_model, newdata = log_model$trainingData, type = "prob")
predicted_probs <- predictions[, "Yes"] 
actual_labels <- cardio_cl$cardio 
...
<<<<<<< HEAD
ggplot(importance_lr, aes(x=reorder(Feature, Estimate), y=Estimate)) +
=======
...
### Combining pressures
We create a new variable that combines the pressure measurements: `ap_hu` is the difference between `ap_lo` and `ap_hi`.
```{r}
cardio$ap_pu = cardio$ap_hi - cardio$ap_lo
...
We can see that they are larger at the extremes: they correspond to the points we previously pointed out in the scatter plot, which represent either individuals with low blood pressure which present heart disease, or those with high blood pressure which donàt have any disease. 
...
It's possible to see that by considering the polynomials of systolic blood pressure, the residuals follow less of a pattern. Further factors (like higher polynomials and multiple interactions) were also considered but they did not contribute to improving the model. 
...
print(cvfit)
```
By using a $\lambda$ which produces a misclassification error within 1 standard error from the minimum's, we obtain similar results ì, while also simplifying the model: those problematic coefficients from before are shrunk to 0, underlying their secondary importance in determining the presence of heart disease. This is indeed consistent with what we had supposed in our initial exploration of the data. 
...
# Get predicted probabilities
predictions <- predict(fits1, newx = X_test, type = "class")
actual_labels <- Y_test

# Manually compute the area under ROC curve
roc_curve <- roc(actual_labels, as.numeric(predictions))

# ROC curve
plot(roc_curve, main = "ROC Curve for LASSO", col = "blue", lwd = 2)
```
```{r}
roc_curve$auc
```
...
# Get predicted probabilities
predictions <- predict(poly3, newdata = poly3$trainingData, type = "prob")
predicted_probs <- predictions[, "Yes"] 
actual_labels <- cardio_cl$cardio 
...
# ROC curve
plot(roc_curve, main = "ROC Curve for poly3", col = "blue", lwd = 2)
```
```{r}
roc_curve$auc
```
...
# Get predicted probabilities
predictions <- predict(poly4, newdata = poly4$trainingData, type = "prob")
predicted_probs <- predictions[, "Yes"] 
actual_labels <- cardio_cl$cardio 
...
# ROC curve
plot(roc_curve, main = "ROC Curve for poly4", col = "blue", lwd = 2)
```
```{r}
roc_curve$auc
```
...
=======
## Other models
We consider the square of age, as as one gets older health is more fragile.
```{r}
age_sq = glm(cardio ~ . - ap_pu + poly(age, degree = 2), data = cardio, family = binomial(link = "logit") )
summary(age_sq)
age_sq$aic
```
...
### Interaction model
```{r}
model_int = glm(cardio ~ . -ap_lo -ap_hi + cholesterol*gluc, data = cardio, family = binomial(link = "logit"))
summary(model_int)
```
### Model diagnostics
```{r}
model_all = glm(cardio ~ . - ap_pu, data = cardio, family = binomial(link = "logit"))
summary(model_all)
print(model_all$aic)
...
vif_values
```
Apparently there is no multicollinearity.
...
br <- binned.resids(pred, res, nclass = 50)$binned
plot(range(br[,1]), range(br[,2],br[,6], -br[,6]), type = "n",
     xlab = "Estimated Pr(Switching)", ylab = "Average residual", main = "Binned Residual Plot")
abline(h = 0, col = "gray", lwd = 0.5); points(br[,1], br[,2], pch = 19, cex = 0.5)
lines(br[,1], br[,6], col = "gray", lwd = 0.5); lines(br[,1], -br[,6], col = "gray", lwd = 0.5)

```
!!! THE RESIDUALS ARE DEFINITELY FOLLOWING A PATTERN

Let's check with the individual variables (not working)
```{r, eval=FALSE}
par(mfrow = c(1, 2))
br.ap_hi <- binned.resids(cardio$ap_hi, res,
                         nclass = 40)$binned
plot(range(br.ap_hi[,1]), range(br.ap_hi[,2], br.ap_hi[,6], -br.ap_hi[,6]), type = "n",
     xlab = "Distance", ylab = "Average residual", main = "Binned residual plot")
abline(h = 0, col = "gray", lwd = 0.5)
lines(br.ap_hi[,1], br.ap_hi[,6], col = "gray", lwd = 0.5)
lines(br.ap_hi[,1], -br.ap_hi[,6], col = "gray", lwd = 0.5)
points(br.ap_hi[,1], br.ap_hi[,2], pch = 19, cex = 0.5)

br.ap_lo <- binned.resids(cardio$ap_lo, res,
                            nclass = 40)$binned
plot(range(br.ap_lo[,1]), range(br.ap_lo[,2], br.ap_lo[,6], -br.ap_lo[,6]), type = "n",
     xlab = "Arsenic concentration", ylab = "Average residual", main = "Binned residual plot")
abline(h = 0, col = "gray", lwd = 0.5)
lines(br.ap_lo[,1], br.ap_lo[,6], col = "gray", lwd = 0.5)
lines(br.ap_lo[,1], -br.ap_lo[,6], col = "gray", lwd = 0.5)
points(br.ap_lo[,1], br.ap_lo[,2], pch = 19, cex = 0.5)
