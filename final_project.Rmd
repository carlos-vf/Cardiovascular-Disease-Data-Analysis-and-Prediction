...
For this models, as well as for the following ones, dataset will be split in two parts: 80% for training the model and 20% for validation. 
...
Another interesting approach to the problem could be fitting some logistic regression models. These models are mainly used when the target variable is dichotomous, as in this case, where only two classes can be predicted (0/1).
...
At first sight, the results obtained are pretty similar to the ones from the previous model. 
...
Let's compute some performance metric:
