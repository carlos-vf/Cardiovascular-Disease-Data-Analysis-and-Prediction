...
predicted_class <- ifelse(predicted_probs >= 0.4, "Yes", "No")
actual_labels <- test_data$cardio
cm <- confusionMatrix(as.factor(predicted_class), as.factor(actual_labels))
...
ggplot(grid_full, aes(x = ap_hi, y = ap_lo, color = predicted_class)) +
  ...
  geom_point(data = test_data, aes(x = ap_hi, y = ap_lo, color = factor(cardio)), linewidth = 2) + 
  ...
  labs(title = "Decision Boundary for Logistic Regression Model with Cholesterol Levels (t=0.4)",
...
3. An increasing of the systolic or diastolic pressure can also negatively affect your health. For each augment in $10$ units to them, we get odds ratios of $1.76$ and $1.10$ respectively. 
