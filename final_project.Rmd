...
roc_curve_rf <- roc(actual_labels, predicted_probs)
...
# Generate grid of unique ap_hi and ap_lo values
grid <- expand.grid(
  ap_hi = seq(min(cardio_cl$ap_hi), max(cardio_cl$ap_hi), length.out = 200),
  ap_lo = seq(min(cardio_cl$ap_lo), max(cardio_cl$ap_lo), length.out = 200)
)

other_predictors <- cardio_cl %>%
...
grid$predicted_probs <- probabilities[, 2]


# Use the grid for contour and tile layers
decision_boundary <- ggplot() +
  ...
  scale_fill_gradient(low = "#FFCCCC", high = "#6699FF", name = "P(cardio=1)") +
  
  # Original data points
  geom_point(data = cardio_cl, aes(x = ap_hi, y = ap_lo, color = as.factor(cardio)), linewidth = 2.5) +
...
ggsave("images/decision_boundary.png", decision_boundary, width = 8, height = 8)
decision_boundary
```
...
# Add a legend
legend("bottomright", legend = c("Random Forest", "Basic Logistic Regression", "Polynomial Regression 3",  "Polynomial Regression 4"), col = c("red", "blue", "green", "orange"), lwd = 2)

# Model results
results <- data.frame(
  Model = c("Random Forest", "Basic Logistic Regression", "Polynomial Regression 3", "Polynomial Regression 4"),
  AUC = c(
...
As it is shown above, the four main models get similar results in the task of classifying the data, all of them with an area under the ROC curve in the range [0.79 - 0.80]. Despite its slightly better performance, the lack of explainability makes the random forest a less suitable model for this context, leaving the logistic regressions as the best choices. 
