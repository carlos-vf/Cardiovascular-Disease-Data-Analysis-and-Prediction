...
png("images/cluster_summary.png", width = 600, height = 400) 
# Convert tibble to a table object
table <- tableGrob(cluster_summary)
grid.draw(table)
dev.off()
```

One can clearly see how the clusters different in terms of health: there is a very high incidence of cardiovascular disease in the first cluster, which has also predictably higher levels of cholesterol and glucose present in their blood. Gender percentages are almost equal, with slightly more females being in the "healthier" cluster.
...
cores <- detectCores()
cl <- makeCluster(cores)  # Create a cluster with all available cores
registerDoParallel(cl)    # Register the parallel backend

# Define a function to compute the distance matrix for a subset of rows
compute_partial_daisy <- function(data, rows, metric) {
  daisy(data[rows, ], metric = metric)
}


# Split the data indices into chunks for parallel processing
chunk_size <- 1000 # Adjust this based on memory and computation limits
indices <- split(1:nrow(cardio_scaled_onlynums), ceiling(seq_along(1:nrow(cardio_scaled_onlynums)) / chunk_size))

sample_cardio_onlynums <- cardio_scaled_onlynums[sample(nrow(cardio_scaled_onlynums), 35000), ]

# Parallel computation of partial distance matrices
partial_distances <- foreach(chunk = indices, .combine = cbind, .packages = "cluster") %dopar% {
  compute_partial_daisy(cardio_scaled_onlynums, chunk, metric = "gower")
}
# Combine partial distance matrices (if needed, depending on format)
dist_matrix_hc <- as.dist(partial_distances)

# Free resources
stopCluster(cl)

hierarchical = hclust(dist_matrix_hc, method = "ward.D2")
