...
# Reduce train_data and test_data to 1% of their original sizes
train_data_small <- train_data[sample(nrow(train_data), size = floor(0.01 * nrow(train_data))), ]
test_data_small <- test_data[sample(nrow(test_data), size = floor(0.01 * nrow(test_data))), ]

# Model matrix creation
train_data_dummies <- model.matrix(cardio ~ ., data = train_data_small)[, -1]  # Exclude intercept column
test_data_dummies <- model.matrix(cardio ~ ., data = test_data_small)[, -1]   # Exclude intercept column

X_train <- scale(train_data_dummies)
Y_train <- as.numeric(train_data_small$cardio) - 1  # Assuming 'cardio' is a factor variable (0/1)
...
test_accuracy_min <- mean(test_pred_classes_min == as.numeric(test_data_small$cardio) - 1)
test_roc_min <- roc(as.numeric(test_data_small$cardio) - 1, as.vector(test_predictions_min))
test_conf_matrix_min <- table(Predicted = test_pred_classes_min, Actual = as.numeric(test_data_small$cardio) - 1)
...
test_accuracy_1se <- mean(test_pred_classes_1se == as.numeric(test_data_small$cardio) - 1)
test_roc_1se <- roc(as.numeric(test_data_small$cardio) - 1, as.vector(test_predictions_1se))
test_conf_matrix_1se <- table(Predicted = test_pred_classes_1se, Actual = as.numeric(test_data_small$cardio) - 1)
...
test_mse_min <- mean((test_predictions_min - (as.numeric(test_data_small$cardio) - 1))^2)
test_mse_1se <- mean((test_predictions_1se - (as.numeric(test_data_small$cardio) - 1))^2)
...
loss_data <- data.frame(lambda = lambda_values, cv_loss = cvm_values)
print(loss_data)
library(ggplot2)
...
# Plot
ggplot(loss_data, aes(x = log_lambda, y = cv_loss)) +
  geom_line() +
  geom_point() +
  labs(
    title = "Loss Function (Log-Likelihood) vs Regularization",
    x = "log(Lambda)",
    y = "Cross-Validated Loss"
  ) +
  theme_minimal()
